...
const Situations17 = () => {
  ...
  const handleAnswerSelect = (value) => {
    // Si ya hay una opción seleccionada, deseleccionarla
    if (answer === value) {
      setAnswer(null);
    } else {
      setAnswer(value);
  ...
  const handleNext = () => {
    ...
    if (answer !== null) {
      ...
      dispatch({ type: 'ADD_ANSWER', questionId: 'situations17', answer: answer });
      navigation.navigate('Situations18');
    } else {
  ...
  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">Por mi edad alguien asumió que no podía comprender algo</Text>
        <FlatList
          data={options}
        ...
        <View className="flex-row space-x-4">
          <TouchableOpacity className="bg-secondary rounded-lg w-24 h-24 justify-center items-center" onPress={() => navigation.navigate('Situations16')}>
