...
const Process8 = () => {
  ...
  const handleNext = () => {
    if (answer !== null) {
      dispatch({ type: 'ADD_ANSWER', questionId: 'process8', answer });
    }
    navigation.navigate('Process9');
  };

  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">La calidad de la vida social que tenga en los siguientes años depende de mí</Text>
        <FlatList
          data={options}
