...
const Process11 = () => {
  ...
  const handleAnswerSelect = (value) => {
    // Si la opción seleccionada es diferente a la respuesta actual, actualiza la respuesta
    if (value !== answer) {
      setAnswer(value);
    } else {
      // Si la opción seleccionada es igual a la respuesta actual, deselecciona la respuesta actual
      setAnswer(null);
    }
  };
  ...
  const handleNext = () => {
    ...
    if (answer !== null) {
      ...
      dispatch({ type: 'ADD_ANSWER', questionId: 'process11', answer });
      navigation.navigate('Process12');
    } else {
  ...
  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">Me deprimo cuando pienso en el efecto que envejecer puede tener en mi vida social</Text>
        <FlatList
          data={options}
        ...
        <View className="flex-row space-x-4">
          <TouchableOpacity className="bg-secondary rounded-lg w-24 h-24 justify-center items-center" onPress={() => navigation.navigate('Process10')}>
