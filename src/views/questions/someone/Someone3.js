...
const Someone3 = () => {
  ...
  const handleAnswerSelect = (value) => {
    ...
    if (answer === value) {
      // Si la opción seleccionada es la misma que la opción actual, deseleccionarla
      setAnswer(null);
    } else {
      // Si la opción seleccionada es diferente a la opción actual, seleccionarla
      setAnswer(value);
    }
  };
  ...
  const handleNext = () => {
    ...
    if (answer !== null) {
      ...
      dispatch({ type: 'ADD_ANSWER', questionId: 'someone3', answer });
      navigation.navigate('Someone4');
    } else {
  ...
  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">Alguien ha estado disponible para darme cariño/afecto</Text>
        <FlatList
          data={options}
        ...
        <View className="flex-row space-x-4">
          <TouchableOpacity className="bg-secondary rounded-lg w-24 h-24 justify-center items-center" onPress={() => navigation.navigate('Someone2')}>
