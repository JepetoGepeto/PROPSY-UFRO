...
const Sleeping2 = () => {
  ...
  const handleAnswerSelect = (value) => {
    // Si la opción seleccionada ya es la respuesta actual, la deseleccionamos
    if (value === answer) {
      setAnswer(null);
    } else {
      setAnswer(value);
  ...
  const handleNext = () => {
    ...
    if (answer !== null) {
      ...
      dispatch({ type: 'ADD_ANSWER', questionId: 'despertar', answer });
      navigation.navigate('Sleeping3');
    } else {
  ...
  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">Se despertó en la mitad de la noche o muy temprano por las mañanas</Text>
        <FlatList
          data={options}
        ...
        <View className="flex-row space-x-4">
          <TouchableOpacity className="bg-secondary rounded-lg w-24 h-24 justify-center items-center" onPress={() => navigation.navigate('Sleeping1')}>
