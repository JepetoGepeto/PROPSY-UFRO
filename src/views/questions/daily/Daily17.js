...
const Daily17 = () => {
  ...
  const handleNext = () => {
    if (answer !== null) {
      dispatch({ type: 'ADD_ANSWER', questionId: 'daily17', answer });
    }
    navigation.navigate('LandingFinal');
  };

  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">Tengo problemas para organizarme si tengo que hacer muchas cosas a la vez</Text>
        <FlatList
          data={options}
