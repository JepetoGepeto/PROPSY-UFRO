...
const Frequency9 = () => {
  ...
  const handleNext = () => {
    if (answer !== null) {
      dispatch({ type: 'ADD_ANSWER', questionId: 'frequency9', answer: answer });
    }
    navigation.navigate('Frequency10');
  };

  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">¿Con qué frecuencia ha sentido que es difícil para usted hacer amigos?</Text>
        <FlatList
          data={options}
