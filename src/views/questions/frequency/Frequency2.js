...
const Frequency2 = () => {
  ...
  const handleNext = () => {
    if (answer !== null) {
      dispatch({ type: 'ADD_ANSWER', questionId: 'frequency2', answer: answer });
    }
    navigation.navigate('Frequency3');
  };

  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">¿Con qué frecuencia ha sentido que no tiene a nadie con quien hablar?</Text>
        <FlatList
          data={options}
        ...
        <View className="flex-row space-x-4">
          <TouchableOpacity className="bg-secondary rounded-lg w-24 h-24 justify-center items-center" onPress={() => navigation.navigate('Frequency1')}>
