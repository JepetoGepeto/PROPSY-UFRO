...
const Frequency8 = () => {
  ...
  const handleNext = () => {
    if (answer !== null) {
      dispatch({ type: 'ADD_ANSWER', questionId: 'frequency8', answer });
    }
    navigation.navigate('Frequency9');
  };

  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">¿Con qué frecuencia ha sentido una gran necesidad de compañía?</Text>
        <FlatList
          data={options}
