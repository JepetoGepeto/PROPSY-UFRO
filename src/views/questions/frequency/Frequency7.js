...
const Frequency7 = () => {
  ...
  const handleNext = () => {
    if (answer !== null) {
      dispatch({ type: 'ADD_ANSWER', questionId: 'frequency7', answer });
    }
    navigation.navigate('Frequency8');
  };

  return (
    <View className="flex-1 bg-[#5F6896] justify-center">
      <View className="flex-1 mt-12 items-center">
        <Text className="text-7xl rotate-180 text-[#3E3E44] bg-primary mb-4 p-4 rounded-lg font-bold">¿Con qué frecuencia se ha sentido incapaz de acercarse a las demás personas y comunicarse con ellas?</Text>
        <FlatList
          data={options}
